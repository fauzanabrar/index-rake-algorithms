...
def generateRake():
    ...
    language = request.form.get("language", "indonesia")  # auto-detect by default
    ...
    # Initialize Sastrawi stemmer for Indonesian if enabled; otherwise skip
    stemmer = None
    if language == 'indonesian':
        if not _STEMMER_ENABLED:
            ...
            try:
                ...
                print("Sastrawi stemmer initialized for Indonesian")
            except Exception as e:
                print(f"Warning: Failed to initialize Sastrawi stemmer: {e}")
                stemmer = None
        else:
            print("Sastrawi library not available; proceeding without stemming")
    ...
    for i in range(len(pages)):
        for j in range(pages[i]["f"], pages[i]["l"]+1):
            ...
            # Use the formatted index for further processing
            ...
            for topNindex in topNindexes:
                ...
                if lower_topNindex not in generated_lower:  # O(1) lookup instead of O(n)
                    # Canonicalize keyword: stem for Indonesian (if enabled), lowercase tokens otherwise
                    if stemmer is not None and language == 'indonesian':
                        # Sastrawi stems full phrase; split into tokens for canonical tuple
